...
public class LoopBlock extends StructuredBlock implements BreakableBlock {
	 ...
	 * :: "(while/do while) with incr"; type == FOR || (incrInstr == null &&
	 * initInstr == null) :: "(while/do while/poss for) with init"; type !=
	 * POSSFOR || incrBlock != null :: "possible for without incr"; type != FOR
	 * || incrInstr != null :: "for without incr"; type != POSSFOR ||
	 * incrBlock.getInstruction() instanceof CombineableOperator ::
	 * "possible for with invalid incr"; initBlock == null ||
	 * (initBlock.getInstruction() instanceof CombinableOperator) ::
	 * "Initializer is not combinableOperator"; initInstr == null || (initInstr
	 * instanceof CombinableOperator) ::
	 * "Initializer is not combinableOperator"; cond != null && cond.getType()
	 * == Type.tBoolean :: "invalid condition type"; type != POSSFOR ||
	 * bodyBlock.contains(incr) :: "incr is not in body of possible for" } }
	 */
	...
	public void setCondition(Expression cond) {
		...
		if (type == POSSFOR) {
			 ...
			 */
			if (cond.containsMatchingLoad((CombineableOperator) incrBlock
					.getInstruction())) {
				type = FOR;
				incrInstr = incrBlock.getInstruction();
				incrBlock.removeBlock();
				if (initBlock != null) {
					if (cond.containsMatchingLoad((CombineableOperator) initBlock
							.getInstruction())) {
				...
				/*
				 * This is not a for block, as it seems first. Make it a while
				 * block again, and forget about init and incr.
				 */
				type = WHILE;
			}
	 ...
	 * This is the initializer for for-blocks.
	 */
		...
		case FOR:
			...
			if (initInstr != null) {
				if (isDeclaration) {
					...
					writer.breakOp();
					writer.print(" = ");
					store.getSubExpressions()[1].makeInitializer(local
							.getType());
	 ...
	 * @return null if there is no way to the end of this block, otherwise the
	 *         stack after the block has executed.
	 */
	 ...
	 * 
	 * @param block
	 *            the breakable block where the breaks originally breaked to
	 *            (Have a break now, if you didn't understand that :-).
	 */
	public void replaceBreakContinue(BreakableBlock block) {
		java.util.Stack todo = new java.util.Stack();
		todo.push(block);
		while (!todo.isEmpty()) {
			StructuredBlock[] subs = ((StructuredBlock) todo.pop())
					.getSubBlocks();
